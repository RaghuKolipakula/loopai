...
export default function App() {
  const [currentPage, setCurrentPage] = useState<'home' | 'validator' | '0dte' | 'seykota'>('home');
      ...
      <div className="max-w-5xl w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          ...
          <div 
            ...
            className="group cursor-pointer bg-slate-900 rounded-2xl p-8 shadow-sm border border-slate-800 hover:shadow-2xl hover:border-amber-500 transition-all duration-300 flex flex-col"
          >
            <div className="bg-amber-900/50 w-16 h-16 rounded-2xl flex items-center justify-center mb-6 text-2xl group-hover:scale-110 transition-transform duration-300">
              🐢
            </div>
            <h2 className="text-2xl font-bold text-white mb-3">Trend Regime Simulator</h2>
            <p className="text-slate-400 mb-6 flex-1">
              Test Seykota-style trend-following mechanics (MA crossovers, ATR position sizing, trailing stops) on real historical market data.
            </p>
            <div className="text-sm font-bold text-amber-400 flex items-center group-hover:translate-x-1 transition-transform">
              Run Simulator &rarr;
            </div>
          </div>
